...
void solve() {
    ...
    while (l <= r) {
        ...
        for (int i = 1; i < n; i++){
            ...
            minL = max(minL, x[i] - mv);
            maxR = min(maxR, x[i] + mv);
        }
        if (!works) {
            l = m+1;
        } else if (minL > maxR) {
    ...
    int mnl = 0; int mxr = INF;
    for (int i = 0; i < n; i++){
  
        mnl = max(mnl, x[i] - (best - t[i]));
        mxr = min(mxr, x[i] + (best - t[i]));
    }

    double rs = (mnl + mxr)/2.0;
