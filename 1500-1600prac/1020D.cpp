...
void solve()
{
    ...
    int minans = 1e9+1;
    for(int i = 1; i <= m; i++) {
        auto i1 = lower_bound(pref.begin(), pref.end(), i-1);
        auto i2 = lower_bound(suff.begin(), suff.end(), m-i);

       
        int ind1 = i1 - pref.begin();
        int ind2 = i2 - suff.begin();
