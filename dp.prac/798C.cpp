...
int dfs2(int val, int parent) {
    vector<int> children = adj[val];
   ...
   vector<int> nchildren = {};
    ...
    if (nchildren.size() == 0) {
        ...
        return chi[nchildren[0]];
    } else {
        int res = max(chi[nchildren[0]] + dfs2(nchildren[1], val), chi[nchildren[1]] + dfs2(nchildren[0], val));
        return res;
    }
