...
void solve()
{
    ...
    int maxp = -1; int maxq = -1;
    int pind = -1; int qind = -1;
    ...
    for(int i = 0; i < n; i++) {
        ...
        int pkicker = q[i - pind];
        int qkicker = p[i - qind];
        
        if (maxp > maxq) {
            res.push_back((pow2[maxp] + pow2[pkicker]) % mod);
        } else if (maxq > maxp) {
            res.push_back((pow2[maxq] + pow2[qkicker]) % mod);
        } else {
            if (pkicker > qkicker) {
                res.push_back((pow2[maxp] + pow2[pkicker]) % mod);
            } else {
