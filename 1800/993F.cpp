...
void solve() {
    ...
    while (q--) {
        ...
        for (int i = 1; i * i <= abs(x); i++) {
            //cout << x << " " << i << " " << x % i << endl;
            if (x% i == 0) {
                if (apos.find(i) != apos.end() && bpos.find(x/i) != bpos.end()) {
                    fnd = true;
                    cout << "YES\n";
                    break;
                }
                if (bpos.find(i) != bpos.end() && apos.find(x/i) != apos.end()) {
                    fnd = true;
                    cout << "YES\n";
                    break;
                }
                if (apos.find(-1 * i) != apos.end() && bpos.find(-1 * (x/i)) != bpos.end()) {
                    fnd = true;
                    cout << "YES\n";
                    break;
                }
                if (bpos.find(-1 * i) != bpos.end() && apos.find(-1 * (x/i)) != apos.end()) {
                    fnd = true;
                    cout << "YES\n";
                    break;
                }
